...
import com.karashok.lib_widget.tab_layout.listener.CustomTabEntity
...
class CommonTabLayout @JvmOverloads constructor(
    ...
    private fun addTab(position: Int, tabView: View) {
        ...
        tv_tab_title.setText(mTabEntitys.get(position).tabTitle)
        val iv_tab_icon = tabView.findViewById(R.id.iv_tab_icon) as ImageView
        iv_tab_icon.setImageResource(mTabEntitys.get(position).tabUnselectedIcon)
    ...
    private fun updateTabStyles() {
        for (i in 0 until tabCount) {
            ...
            if (mIconVisible) {
                ...
                val tabEntity = mTabEntitys.get(i)
    ...
    fun setMsgMargin(position: Int, leftPadding: Float, bottomPadding: Float) {
        ...
        if (tipView != null) {
            ...
            if (mIconVisible) {
                if (iconH <= 0) {
                    iconH = mContext.resources.getDrawable(mTabEntitys.get(position).tabSelectedIcon)
